...
VkPixelDescriptorPool::VkPixelDescriptorPool(const VkPixelDevice::Ptr &device) {
    mDevice = device;
}

VkPixelDescriptorPool::~VkPixelDescriptorPool() {
    if (mPool != VK_NULL_HANDLE) {
        vkDestroyDescriptorPool(mDevice->getDevice(), mPool, nullptr);
    }
}

void VkPixelDescriptorPool::build(std::vector<UniformParameter::Ptr>& params, const int& frameCount) {
    //decriptor
