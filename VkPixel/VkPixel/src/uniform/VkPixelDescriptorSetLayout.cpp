//
//  VkPixelDescriptorSetLayout.cpp
//  VkPixel
//
...
VkPixelDescriptorSetLayout::VkPixelDescriptorSetLayout(const VkPixelDevice::Ptr &device) {
    mDevice = device;
}

VkPixelDescriptorSetLayout::~VkPixelDescriptorSetLayout() {
    if (mLayout != VK_NULL_HANDLE) {
        vkDestroyDescriptorSetLayout(mDevice->getDevice(), mLayout, nullptr);
    }
}

void VkPixelDescriptorSetLayout::build(const std::vector<UniformParameter::Ptr>& params) {
    mParams = params;
