...
#include "VkPixelPipeline.h"

namespace vkpixel {

VkPixelPipeline::VkPixelPipeline(const VkPixelDevice::Ptr& device, const VkPixelRenderPass::Ptr& renderPass) {
    mDevice = device;
...
VkPixelPipeline::~VkPixelPipeline() {
...
void VkPixelPipeline::setShaderGroup(const std::vector<VkPixelShader::Ptr>& shaderGroup) {
    mShaders = shaderGroup;
}

void VkPixelPipeline::build() {
    //设置shader
    ...
    //pipeline cache，可以将相关数据存入缓存，在多个pipeline当中使用,也可以存到文件，不同程序调用s
