...
void VkApplication::initVulkan() {
    mInstance = VkPixelInstace::create(true);
    mSurface = VkPixelSurface::create(mInstance, mWindow);

    mDevice = VkPixelDevice::create(mInstance, mSurface);

    mSwapChain = VkPixelSwapChain::create(mDevice, mWindow, mSurface);
    mWidth = mSwapChain->getExtent().width;
    mHeight = mSwapChain->getExtent().height;

    mRenderPass = VkPixelRenderPass::create(mDevice);
    createRenderPass();
    ...
    //创建模型
    mModel = VkPixelModel::create(mDevice);

    mPipeline = VkPixelPipeline::create(mDevice, mRenderPass);
    createPipeline();
...
void VkApplication::createPipeline() {
    ...
    //设置shader
    std::vector<VkPixelShader::Ptr> shaderGroup{};

    auto shaderVertex = VkPixelShader::create(mDevice, "/Users/jason/Jason/project/vulkan-tutorial/VkPixel/VkPixel/src/shader/vs.spv", VK_SHADER_STAGE_VERTEX_BIT, "main");
    shaderGroup.push_back(shaderVertex);

    auto shaderFragment = VkPixelShader::create(mDevice, "/Users/jason/Jason/project/vulkan-tutorial/VkPixel/VkPixel/src/shader/fs.spv", VK_SHADER_STAGE_FRAGMENT_BIT, "main");
...
void VkApplication::createSyncObjects() {
    for (int i = 0; i < mSwapChain->getImageCount(); ++i) {
        auto imageSemaphore = VkPxielSemaphore::create(mDevice);
        mImageAvailableSemaphores.push_back(imageSemaphore);

        auto renderSemaphore = VkPxielSemaphore::create(mDevice);
